...
public class TaxRepositoryTest {
    ...
    public void getTaxRateObject() {
        Optional<Tax> resultTax;

        for (Tax stateSalesTax :
                states) {
            resultTax = taxRepository.findById(stateSalesTax.getState());

            assertTrue(resultTax.isPresent());
            assertEquals(resultTax.get(), stateSalesTax);
        }
    }
    ...
    public void getTaxRate() {
        ...
        for (Tax stateSalesTax :
            ...
            resultTax = taxRepository.findById(stateSalesTax.getState());
            assertTrue(resultTax.isPresent());
            assertEquals(resultTax.get().getRate(), stateSalesTax.getRate());
        }
    }
}